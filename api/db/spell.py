...
class SpellDB:
    ...
    def get_spell_raw_data(self, spell_id=None, spell_name=None, spell_class=None):
        ...
        CLASS_COLUMN_MAP = {
            ...
            'paladin': 'classes3',
            'ranger': 'classes4',
            'shadowknight': 'classes5',
            'druid': 'classes6',
            'monk': 'classes7',
            'bard': 'classes8',
            'rogue': 'classes9',
            'shaman': 'classes10',
            'necromancer': 'classes11',
            'wizard': 'classes12',
            'magician': 'classes13',
            'enchanter': 'classes14',
            'beastlord': 'classes15',
        ...
        with self.engine.connect() as conn:
            if spell_id:
                ...
                class_column = CLASS_COLUMN_MAP.get(spell_class.lower())
                if not class_column:
                    return []
                query = text(f"""
                    SELECT * FROM spells_new 
                    WHERE `{class_column}` > 0 AND `{class_column}` < 255
                    LIMIT 50
                """)
    ...
    def get_spells_by_class_api(self, class_name, min_level=1, max_level=65):
        CLASS_COLUMN_MAP = {
            'warrior': ('classes1', 1),
            'cleric': ('classes2', 2),
            'paladin': ('classes3', 3),
            'ranger': ('classes4', 4),
            'shadowknight': ('classes5', 5),
            'druid': ('classes6', 6),
            'monk': ('classes7', 7),
            'bard': ('classes8', 8),
            'rogue': ('classes9', 9),
            'shaman': ('classes10', 10),
            'necromancer': ('classes11', 11),
            'wizard': ('classes12', 12),
            'magician': ('classes13', 13),
            'enchanter': ('classes14', 14),
        ...
        with self.engine.connect() as conn:
            query = text(f"""
                SELECT id, name, skill, targettype, new_icon, 
                    ...
                    {', '.join([f'effect_base_value{i}' for i in range(1,13)])},
                    {', '.join([f'effect_limit_value{i}' for i in range(1,13)])},
                    {', '.join([f'max{i}' for i in range(1,13)])},
                    {', '.join([f'formula{i}' for i in range(1,13)])},
                    buffduration, teleport_zone, buffdurationformula, {class_column}
                FROM spells_new
                WHERE `{class_column}` >= :min_level AND `{class_column}` <= :max_level
                ORDER BY `{class_column}`, name
            """)
            results = conn.execute(query, {'min_level': min_level, 'max_level': max_level}).fetchall()
            data = {'game_class': get_spell_class(class_id)}
            for entry in results:
                entry = entry._mapping
                spell_id = entry['id']
                if spell_id in excl_list:
                    continue
                level = entry[class_column]
                ...
                for idx in range(1, 13):
                    if entry[f'effectid{idx}'] != 254:
                        slots[f'slot_{idx}'] = self.parse_slot_data(idx, entry, no_item_links=True)
    ...
    def parse_slot_data(self, idx, data, no_item_links=False):
        ...
        ret_data = {'spa': spa,
                    ...
                    'formula': formula}
        ret_data.update({'desc': self.translate_spa(spa, min_val, limit_val, formula,
                                            max_val, min_level, data, no_item_links)})
        return ret_data    

    def translate_spa(self, spa, min_val, limit_val, formula, max_val, min_level, data, no_item_links=False):
        if spa == 0:
            # HP
            # Handle Formula = 122
            if formula == 122:
            ...
            if data.teleport_zone == 'same':
                ...
                zone_id, zone_name = zone_db.get_zone_long_name(data.teleport_zone)
            ...
            spell_name = self.get_spell_name(min_val)
            ...
            spell_name = self.get_spell_name(abs(min_val))
            if min_val >= 0:
            ...
            # Focus: Proc on Cast
            spell_name = self.get_spell_name(abs(limit_val))
...
def get_spell_min_level(data, class_id=None):
    """
    Returns the minimum level required for a spell for a given class.
    If class_id is None, tries to infer from data.
    """
    if class_id is not None:
        class_col = f'classes{class_id}'
        return data.get(class_col, None)
    # If class_id is not provided, try to find the minimum among all class columns
    min_level = None
    for i in range(1, 17):
        level = data.get(f'classes{i}', None)
        if level is not None and (min_level is None or level < min_level):
            min_level = level
    return min_level

def get_spell_class(class_id):
    class_map = {
        1: "Warrior",
        2: "Cleric",
        3: "Paladin",
...
def calculate_values(base, start_level, max_val, test_value_func, ignore_max=False):
    """
    Calculates the value for a given base, level, and max using the provided test_value_func.
    Returns the value and the max level reached.
    """
    value = test_value_func(base, start_level)
    max_level = start_level
    if ignore_max:
        return value, max_level
    # Try to find the level where the value reaches or exceeds max_val
    for lvl in range(start_level, 256):  # 255 is a common level cap
        v = test_value_func(base, lvl)
        if v >= max_val:
            max_level = lvl
            value = v
            break
