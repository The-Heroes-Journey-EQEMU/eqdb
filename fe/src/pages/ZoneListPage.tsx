    ...
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center border-b border-gray-600">
        ...
        <div className="flex items-center space-x-2 pr-4">
          ...
          <select value={minLevel} onChange={handleMinLevelChange} className="bg-gray-700 text-white rounded p-1">
            {minLevelOptions.filter(level => level < maxLevel - 5).map(level => (
              <option key={level} value={level}>{level}</option>
            ))}
          </select>
          <span>-</span>
          <select value={maxLevel} onChange={handleMaxLevelChange} className="bg-gray-700 text-white rounded p-1">
